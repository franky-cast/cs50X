...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // update votes 2d array if candidate found
...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++) 
        {
            ...
            // if candidate not eliminated, update that candidates vote && move on to next voter (i +1)
            if (!candidates[candidate_index].eliminated)
            {
                candidates[candidate_index].votes++;
                break;
            }
            // else check next voter preference (move on to next iteration of loop)
        }
    }
    return;
}
